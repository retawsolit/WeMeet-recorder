...
const recorderUserAuthName = "WEMEET_RECORDER_AUTH"
...
func NewNatsConnection(appCnf *config.AppConfig) error {
	tokenHandler := func() string {
		...
		token, err := auth.GenerateWeMeetJWTAccessToken(appCnf.WeMeetInfo.ApiKey, appCnf.WeMeetInfo.ApiSecret, c.UserId, time.Minute*5, c)
		if err != nil {
