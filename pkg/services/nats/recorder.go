...
func (s *NatsService) AddRecorder() error {
	...
	data := map[string]string{
		fmt.Sprintf("%d", wemeet.RecorderInfoKeys_RECORDER_INFO_MAX_LIMIT):        fmt.Sprintf("%d", s.app.Recorder.MaxLimit),
		fmt.Sprintf("%d", wemeet.RecorderInfoKeys_RECORDER_INFO_CURRENT_PROGRESS): "0",
		fmt.Sprintf("%d", wemeet.RecorderInfoKeys_RECORDER_INFO_LAST_PING):        fmt.Sprintf("%d", time.Now().UTC().UnixMilli()),
	}
...
func (s *NatsService) UpdateLastPing() error {
	...
	_, err = kv.PutString(s.ctx, fmt.Sprintf("%d", wemeet.RecorderInfoKeys_RECORDER_INFO_LAST_PING), fmt.Sprintf("%d", time.Now().UTC().UnixMilli()))
	if err != nil {
		return err
	}
...
func (s *NatsService) UpdateCurrentProgress(increment bool) error {
	...
	key := fmt.Sprintf("%d", wemeet.RecorderInfoKeys_RECORDER_INFO_CURRENT_PROGRESS)
